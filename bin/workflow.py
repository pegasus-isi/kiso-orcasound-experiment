...
class OrcasoundWorkflow:
    ...
    # --- Init ---------------------------------------------------------------------
    def __init__(
        self, sensors, start_date, end_date, max_files, dagfile="workflow.yml"
    ):
        self.dagfile = dagfile
        self.wf_dir = str(Path(__file__).parent.parent.resolve())
    ...
    def create_transformation_catalog(self, exec_site_name="condorpool"):
        ...
        orcasound_container = Container(
            "orcasound_container",
            container_type=Container.SINGULARITY,
            image="docker://pegasus/orcasound-processing:latest",
            image_site="docker_hub",
        )

        orcasound_ml_container = Container(
            "orcasound_ml_container",
            container_type=Container.SINGULARITY,
            image="docker://pegasus/orcasound-ml-processing:latest",
            image_site="docker_hub",
        )
        ...
        convert2wav = Transformation(
            "convert2wav",
            site=exec_site_name,
            pfn=os.path.join(self.wf_dir, "bin/convert2wav.py"),
            is_stageable=True,
            container=orcasound_container,
        ).add_pegasus_profile(memory="1 GB")
        convert2spectrogram = Transformation(
            "convert2spectrogram",
            site=exec_site_name,
            pfn=os.path.join(self.wf_dir, "bin/convert2spectrogram.py"),
            is_stageable=True,
            container=orcasound_container,
        ).add_pegasus_profile(memory="1 GB")
        inference = Transformation(
            "inference",
            site=exec_site_name,
            pfn=os.path.join(self.wf_dir, "bin/inference.py"),
            is_stageable=True,
            container=orcasound_ml_container,
        ).add_pegasus_profile(memory="1 GB")
        ...
        self.tc.add_transformations(
            convert2wav, convert2spectrogram, inference, merge, mkdir
        )
    ...
    def fetch_s3_catalog(self):
        ...
        if data.status_code != 200:
            raise ConnectionError(
                "Download for {} failed with error code: {}".format(
                    self.s3_cache_xz_url, data.status_code
                )
            )
    ...
    def check_s3_cache(self):
        s3_files = self.s3_cache[
            self.s3_cache["Sensor"].isin(self.sensors)
            & (self.s3_cache["Timestamp"] >= self.start_date)
            & (self.s3_cache["Timestamp"] <= self.end_date)
        ]

        if s3_files.empty:
            print(
                "No files found for sensors between {} and {}".format(
        ...
        for sensor in self.sensors:
            if s3_files[s3_files["Sensor"] == sensor].empty:
                print(
                    "No files found for sensor {} between {} and {}".format(
                        sensor, self.start_date, self.end_date
                    )
                )
    ...
    def create_replica_catalog(self):
        ...
        self.rc.add_replica(
            "local", "model.py", os.path.join(self.wf_dir, "bin/model.py")
        )
        self.rc.add_replica(
            "local", "dataloader.py", os.path.join(self.wf_dir, "bin/dataloader.py")
        )
        self.rc.add_replica(
            "local", "params.py", os.path.join(self.wf_dir, "bin/params.py")
        )
        self.rc.add_replica(
            "local", "model.pkl", os.path.join(self.wf_dir, "input/model.pkl")
        )
    ...
    def create_workflow(self):
        ...
        for sensor in self.sensors:
            ...
            for ts in self.s3_files[self.s3_files["Sensor"] == sensor][
                "Timestamp"
            ].unique():
                predictions_sensor_ts_files = []
                sensor_ts_files = self.s3_files[
                    (self.s3_files["Sensor"] == sensor)
                    & (self.s3_files["Timestamp"] == ts)
                    & (self.s3_files["Filename"] != "live.m3u8")
                ]
                sensor_ts_files_len = len(sensor_ts_files.index)
                # -2 if m3u8 in the list else -1
                sensor_ts_files = sensor_ts_files[
                    sensor_ts_files["Filename"]
                    != "live{}.ts".format(sensor_ts_files_len - 1)
                ]
                ...
                mkdir_job = (
                    Job(
                        "mkdir",
                        _id="scratch_mkdir_{0}_{1}".format(sensor, ts),
                        node_label="scratch_mkdir_{0}_{1}".format(sensor, ts),
                    )
                    .add_args(
                        "-p ${0}/png/{1}/{2} ${0}/wav/{1}/{2}".format(
                            "_PEGASUS_INITIAL_DIR", sensor, ts
                        )
                    )
                    .add_profiles(
                ...
                for job_files in np.array_split(sensor_ts_files, num_of_splits):
                    ...
                    for f in job_files["Filename"]:
                        wav_files.append(
                            "wav/{0}/{1}/{2}".format(
                                sensor, ts, f.replace(".ts", ".wav")
                            )
                        )
                        png_files.append(
                            "png/{0}/{1}/{2}".format(
                                sensor, ts, f.replace(".ts", ".png")
                            )
                        )
                    ...
                    convert2wav_job = (
                        Job(
                            "convert2wav",
                            _id="wav_{0}_{1}_{2}".format(sensor, ts, counter),
                            node_label="wav_{0}_{1}_{2}".format(sensor, ts, counter),
                        )
                        .add_args("-i {0}/hls/{1} -o wav/{0}/{1}".format(sensor, ts))
                        .add_inputs(*input_files, bypass_staging=True)
                        .add_outputs(
                            *wav_files, stage_out=False, register_replica=False
                        )
                        .add_pegasus_profiles(
                            label="{0}_{1}_{2}".format(sensor, ts, counter)
                        )
                    )

                    convert2spectrogram_job = (
                        Job(
                            "convert2spectrogram",
                            _id="png_{0}_{1}_{2}".format(sensor, ts, counter),
                            node_label="spectrogram_{0}_{1}_{2}".format(
                                sensor, ts, counter
                            ),
                        )
                    ...
                    predictions = File(
                        "predictions_{0}_{1}_{2}.json".format(sensor, ts, counter)
                    )
                    predictions_sensor_ts_files.append(predictions)
                    inference_job = (
                        Job(
                            "inference",
                            _id="predict_{0}_{1}_{2}".format(sensor, ts, counter),
                            node_label="inference_{0}_{1}_{2}".format(
                                sensor, ts, counter
                            ),
                        )
                        .add_args(
                            "-i wav/{0}/{1} -s {0} -t {1} -m {3} -o predictions_{0}_{1}_{2}.json".format(
                                sensor, ts, counter, model_file.lfn
                            )
                        )
                        .add_inputs(
                            model_file, model_py, dataloader_py, params_py, *wav_files
                        )
                        .add_outputs(
                            predictions, stage_out=False, register_replica=False
                        )
                        .add_pegasus_profiles(
                            label="{0}_{1}_{2}".format(sensor, ts, counter)
                        )
                    ...
                    # Share files to jobs
                    self.wf.add_jobs(
                        convert2wav_job, convert2spectrogram_job, inference_job
                    )
                    self.wf.add_dependency(
                        mkdir_job, children=[convert2wav_job, convert2spectrogram_job]
                    )
                ...
                merge_job_ts = (
                    Job(
                        "merge",
                        _id="merge_{0}_{1}".format(sensor, ts),
                        node_label="merge_{0}_{1}".format(sensor, ts),
                    )
                    .add_args(
                        "-i {0} -o {1}".format(
                            " ".join([x.lfn for x in predictions_sensor_ts_files]),
                            merged_predictions.lfn,
                        )
                    )
                    .add_inputs(*predictions_sensor_ts_files)
                    .add_outputs(
                        merged_predictions, stage_out=True, register_replica=False
                    )
            ...
            if len(predictions_sensor_files) > 1:
                ...
                merge_job_sensor = (
                    Job(
                        "merge",
                        _id="merge_{0}".format(sensor, ts),
                        node_label="merge_{0}".format(sensor, ts),
                    )
                    .add_args(
                        "-i {0} -o {1}".format(
                            " ".join([x.lfn for x in predictions_sensor_files]),
                            merged_predictions.lfn,
                        )
                    )
        ...
        if len(predictions_files) > 1:
            ...
            merge_job_all = (
                Job(
                    "merge",
                    _id="merge_all".format(sensor, ts),
                    node_label="merge_all".format(sensor, ts),
                )
                .add_args(
                    "-i {0} -o {1}".format(
                        " ".join([x.lfn for x in predictions_files]),
                        merged_predictions.lfn,
                    )
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Max files per job (default: 200)",
    )
    parser.add_argument(
        "--sensors",
        metavar="STR",
        type=str,
        choices=["rpi_bush_point", "rpi_port_townsend", "rpi_orcasound_lab"],
        required=True,
        nargs="+",
        help="Sensor source [rpi_bush_point, rpi_port_townsend, rpi_orcasound_lab]",
    )
    parser.add_argument(
        "--start-date",
        metavar="STR",
        ...
        required=True,
        help="Start date (example: '2021-08-10')",
    )
    parser.add_argument(
        "--end-date",
        metavar="STR",
